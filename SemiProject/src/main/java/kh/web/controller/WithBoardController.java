...
public class WithBoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		WithBoardDAO dao = WithBoardDAO.getInstance();
...
//	}
		...
		try {
			if (cmd.equals("/list.with")) {
				...
				List<WithBoardDTO> list = dao.selectByBound(start, end);
				String navi = dao.getPageNavi(currentPage);
		
				request.setAttribute("navi", navi);
				request.setAttribute("list", list);
				request.getRequestDispatcher("/withBoard/list.jsp").forward(request, response);

				// '맛집 공유 게시판'글쓰기 페이지로 넘어가기
			} else if (cmd.equals("/write.with")) {
				response.sendRedirect("/withBoard/write.jsp");

				// '맛집 공유 게시판' 글 작성할때 db로 저장시키는 역할.
			} else if (cmd.equals("/withProc.with")) {
				HttpSession session = request.getSession();
                String writer = (String) request.getSession().getAttribute("loginID"); 
				 // (String) request.getSession().getAttribute("loginID"); 나중에 아이디 추가
				String title = request.getParameter("title");
				String content = request.getParameter("content");

				int result = dao.insert(new WithBoardDTO(0, writer, title, content, null, 0));
				
				if (result > 0) {
					response.sendRedirect("/list.with?cpage=1");
				}

				// 게시글 클릭 시 보여주는 페이지
			} else if (cmd.equals("/detail.with")) {
				int seq = Integer.parseInt(request.getParameter("seq"));
				WithBoardDTO dto = dao.selectBySeq(seq);
				int result = dao.addViewCount(seq);
				int replycount = dao.totalReplyCount(seq);
				
				List<WithBoardReplyDTO> list = dao.selectReplyByPseq(seq);

				request.setAttribute("dto", dto);
				request.setAttribute("list", list);
				request.setAttribute("replycount", replycount);
				request.getRequestDispatcher("/withBoard/detail.jsp").forward(request, response);
				
				// 게시글 삭제 기능
			} else if (cmd.equals("/delete.with")) {
				int seq = Integer.parseInt(request.getParameter("seq"));
			...
			} else if (cmd.equals("/modify.with")) {
				int seq = Integer.parseInt(request.getParameter("seq"));
				WithBoardDTO dto = dao.selectBySeq(seq);
				request.setAttribute("dto", dto);
				request.getRequestDispatcher("/withBoard/modify.jsp").forward(request, response);

				// 'summernote'로 글 수정 하는 페이지
			} else if (cmd.equals("/modifyProc.with")) {
				String title = request.getParameter("title");
				String contents = request.getParameter("content");
				int seq = Integer.parseInt(request.getParameter("seq"));
				int result = dao.modify(seq, title, contents);
				response.sendRedirect("/detail.with?seq=" + seq);

				//// 댓글 등록
			} else if (cmd.equals("/reply.with")) {
				int seq = Integer.parseInt(request.getParameter("seq"));
				HttpSession session = request.getSession();
				String writer = (String)request.getSession().getAttribute("loginID");
				...
				/// 댓글 삭제
			} else if (cmd.equals("/deleteReply.with")) {
				int seq = Integer.parseInt(request.getParameter("seq"));
				...
				/// 댓글 수정
			} else if (cmd.equals("/modifyReply.with")) {
				int seq = Integer.parseInt(request.getParameter("seq"));
				...
				/// 게시글 검색 기능
			} else if (cmd.equals("/search.with")) {
				String cpage = request.getParameter("cpage");
				if (cpage == null)
					cpage = "1";
				...
				List<WithBoardDTO> list = dao.searchBoard(start, end, keyword, searchWord);
				String navi = dao.getPageNaviBySearch(currentPage, keyword, searchWord);
				request.setAttribute("navi", navi);
				request.setAttribute("list", list);
				request.getRequestDispatcher("/withBoard/searchList.jsp").forward(request, response);
	
			// 파일 업로드
			} else if (cmd.equals("/uploadFile.with")) {
