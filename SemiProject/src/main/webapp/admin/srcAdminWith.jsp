...
<jsp:include page="adminSideBar.jsp"></jsp:include>
	...
	<form action="/searchWith.admin?cpage=1" method=post>
		...
		<c:otherwise>
		<form action="/deleteWith.admin" method=post id="frm">
		<c:forEach var="dto" items="${list}">
			<div class=row style="text-align:center;">
				<div class="col-xs-12 col-sm-1"><input type=checkbox name="check" value="${dto.with_seq }"></div>
				<div class="col-xs-12 col-sm-1" id=seq>${dto.with_seq }</div>
				<div class="col-xs-12 col-sm-1">같이먹을사람</div>
				<div class="col-xs-12 col-sm-3" id=title>${dto.with_title }</div>
				<div class="col-xs-12 col-sm-1" id=writer>${dto.with_writer }</div>
				<div class="col-xs-12 col-sm-2" id=write_date>${dto.with_writedate }</div>
				<div class="col-xs-12 col-sm-1" id=view_count>${dto.with_viewcount }</div>
		   		<div class="col-xs-12 col-sm-2">
		   			<a href='/detail.with?seq=${dto.with_seq}'><input type=button value=보기></a>
		   			<a href='/modWith.admin?seq=${dto.with_seq}'><input type=button value=수정 ></a>
		   		</div>
			</div>
		</c:forEach>
		</c:otherwise>
