...
<jsp:include page="adminSideBar.jsp"></jsp:include>
	...
	<form action="/searchShare.admin?cpage=1" method=post>
		...
		<c:otherwise>
		<form action="/deleteShare.admin" method=post id="frm">
		<c:forEach var="dto" items="${list}">
			<div class=row style="text-align:center;">
				<div class="col-xs-12 col-sm-1"><input type=checkbox name="check" value="${dto.sh_seq }"></div>
				<div class="col-xs-12 col-sm-1" id=seq>${dto.sh_seq }</div>
				<div class="col-xs-12 col-sm-1">맛집 공유</div>
				<div class="col-xs-12 col-sm-3" id=title>${dto.sh_title }</div>
				<div class="col-xs-12 col-sm-1" id=writer>${dto.sh_writer }</div>
				<div class="col-xs-12 col-sm-2" id=write_date>${dto.sh_writedate }</div>
				<div class="col-xs-12 col-sm-1" id=view_count>${dto.sh_viewcount }</div>
		   		<div class="col-xs-12 col-sm-2">
		   			<a href='/detail.board?seq=${dto.sh_seq}'><input type=button value=보기></a>
		   			<a href='/modShare.admin?seq=${dto.sh_seq}'><input type=button value=수정 ></a>
		   		</div>
			</div>
		</c:forEach>
		</c:otherwise>
