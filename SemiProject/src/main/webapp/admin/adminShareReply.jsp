...
<jsp:include page="adminSideBar.jsp"></jsp:include>
	<div class="container">
	<form action="/searchShareReply.admin?cpage=1" method=post>
		<div class="row">
			<div class="col-xs-12 col-sm-1" style="text-align:left;padding:0px;">
				<select style="width:100%;height:100%;" name=option>
					<option value="sh_replyseq">댓글번호</option>
					<option value="sh_replypseq">글번호</option>
					<option value="sh_replytext">내용</option>
					<option value="sh_replyer">작성자</option>
				</select>
			</div>
		...
		<c:choose>
			...
			<c:otherwise>
		<form action="/deleteShareCnt.admin" method=post id=frm>
		<c:forEach var="dto" items="${list}">
			<div class=row style="text-align:center;">
				<div class="col-xs-12 col-sm-1"><input type=checkbox name="check" value="${dto.sh_replyseq }"></div>
				<div class="col-xs-12 col-sm-1">${dto.sh_replyseq }</div>
				<div class="col-xs-12 col-sm-1">${dto.sh_replypseq }</div>
				<div class="col-xs-12 col-sm-1">맛집 공유</div>
				<div class="col-xs-12 col-sm-1">${dto.sh_replyer }</div>
				<div class="col-xs-12 col-sm-4">${dto.sh_replytext }</div>
				<div class="col-xs-12 col-sm-2">${dto.sh_replydate }</div>
		   		<div class="col-xs-12 col-sm-1">
		   			<a href='/detail.board?seq=${dto.sh_replypseq}'><input type=button value=보기></a>
		   		</div>
			</div>
		</c:forEach>
		</c:otherwise>
