...
<jsp:include page="adminSideBar.jsp"></jsp:include>
	...
	<form action="/searchRecipe.admin?cpage=1" method=post>
		...
		<c:otherwise>
		<form action="/deleteRecipe.admin" method=post id="frm">
		<c:forEach var="dto" items="${list}">
			<div class=row style="text-align:center;">
				<div class="col-xs-12 col-sm-1"><input type=checkbox name="check" value="${dto.rc_seq }"></div>
				<div class="col-xs-12 col-sm-1" id=seq>${dto.rc_seq }</div>
				<div class="col-xs-12 col-sm-1">레시피</div>
				<div class="col-xs-12 col-sm-3" id=title>${dto.rc_title }</div>
				<div class="col-xs-12 col-sm-1" id=writer>${dto.rc_writer }</div>
				<div class="col-xs-12 col-sm-2" id=write_date>${dto.rc_writedate }</div>
				<div class="col-xs-12 col-sm-1" id=view_count>${dto.rc_viewcount }</div>
		   		<div class="col-xs-12 col-sm-2">
		   			<a href='/detail.recipe?seq=${dto.rc_seq}'><input type=button value=보기></a>
		   			<a href='/modRecipe.admin?seq=${dto.rc_seq}'><input type=button value=수정 ></a>
		   		</div>
			</div>
		</c:forEach>
		</c:otherwise>
