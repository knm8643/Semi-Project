...
<jsp:include page="adminSideBar.jsp"></jsp:include>
	<form action="/modifyRecipe.admin" method="post" id="frmDetail">
		<div class=container>
			<div class=row id=article>
				<input type=hidden value="${dto.rc_seq}" name="seq">
				<div class=col-12 id=title><input type=text id=title name=title placeholder="글 제목을 입력하세요" size=100% value="${dto.rc_title }"></div>
				<div class=col-12 id=write_date>${dto.rc_writedate }</div>
				<div class=col-12 id=content>
					<textarea rows=25 cols=127 id="summernote" name=contents placeholder="글 내용을 입력하세요" readonly>${dto.rc_contents }</textarea>
				</div>
	...
	<script>
		$("#del").on("click",function(){
			if(confirm("정말 삭제하시겠습니까?")){
				location.href="/delete.notice?seq=${dto.rc_seq}";
			}
		})
		...
		$("#modCancel").on("click",function(){
			if(confirm("정말 취소하시겠습니까?")){
				location.href="/recipe.admin?cpage=1";
			}
		});
