...
<jsp:include page="adminSideBar.jsp"></jsp:include>
	<div class="container">
	<form action="/searchQnAReply.admin?cpage=1" method=post>
		<div class="row">
			<div class="col-xs-12 col-sm-1" style="text-align:left;padding:0px;">
				<select style="width:100%;height:100%;" name=option>
					<option value="qna_reply_seq">댓글번호</option>
					<option value="qna_reply_parentseq">글번호</option>
					<option value="qna_reply_title">제목</option>
					<option value="qna_reply_writer">작성자</option>
				</select>
			</div>
		...
		<c:choose>
			...
			<c:otherwise>
		<form action="/deleteQnACnt.admin" method=post id=frm>
			...
			<div class=row style="text-align:center;">
				...
				<div class="col-xs-12 col-sm-2">${dto.write_date }</div>
		   		<div class="col-xs-12 col-sm-1">
		   			<a href='/detail.qna?seq=${dto.parentSeq}'><input type=button value=보기></a>
		   		</div>
			</div>
		</c:forEach>
		</c:otherwise>
