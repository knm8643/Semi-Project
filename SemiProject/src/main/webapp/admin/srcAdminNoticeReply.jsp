...
<jsp:include page="adminSideBar.jsp"></jsp:include>
	<div class="container">
	<form action="/searchNoticeReply.admin?cpage=1" method=post>
		<div class="row">
			<div class="col-xs-12 col-sm-1" style="text-align:left;padding:0px;">
				<select style="width:100%;height:100%;" name=option>
					<option value="noti_reply_seq">댓글번호</option>
					<option value="noti_reply_parentseq">글번호</option>
					<option value="noti_reply_contents">내용</option>
					<option value="noti_reply_writer">작성자</option>
				</select>
			</div>
		...
		<div class="row" style="background-color:#e1e1e1;border-top:1px solid black;border-bottom:1px solid black;">
			...
			<div class="col-xs-12 col-sm-3">내용</div>
		...
		<c:choose>
			...
			<c:otherwise>
		<form action="/deleteCnt.admin" method=post id=frm>
		<c:forEach var="dto" items="${list}">
			<div class=row style="text-align:center;">
				<div class="col-xs-12 col-sm-1"><input type=checkbox name="check" value="${dto.seq }"></div>
				<div class="col-xs-12 col-sm-1">${dto.seq }</div>
				<div class="col-xs-12 col-sm-1">${dto.parentSeq }</div>
				<div class="col-xs-12 col-sm-1">공지사항</div>
				<div class="col-xs-12 col-sm-1">${dto.writer }</div>
				<div class="col-xs-12 col-sm-3">${dto.contents }</div>
				<div class="col-xs-12 col-sm-2">${dto.write_date }</div>
		   		<div class="col-xs-12 col-sm-2">
		   			<a href='/detail.notice?seq=${dto.parentSeq}'><input type=button value=보기></a>
		   		</div>
			</div>
		</c:forEach>
		</c:otherwise>
