...
<body>
	...
	<div class="container" style="padding: 10px;">
		<form action="/modify.with" method="post">
			...
			<input type="hidden" value="${dto.with_seq }" name="seq">

			<!-- 게시글 제목-->
			<div class="row">
				<div class="col">
					<input type="text" id="title" name="title" placeholder="제목을 입력하세요." value="${dto.with_title }" style="margin: 10px; width: 100%; border: none; font-size: 15px;">
				</div>
			</div>

			<!-- 게시글 내용 -->
			<div class="row" style="border: 1px solid gray;">
				<div class="col" id="content" name="content" style="overflow: auto; width: 100%; height: 500px;">${dto.with_contents }</div>
			</div>
			...
			<div class="row" style="margin: 20px; text-align: right;">
				<div class="col" style="float: right;">
					...
					<!--  글쓴 사람한테만 삭제하기 버튼 보여주기 -->
					<c:if test="${dto.with_writer == loginID }">
	...
	<div class="container">
		...
		<c:forEach var="dto" items="${list}">
			<table class="table table-striped">
				<tr>
					<td>
						<input type=hidden value="${dto.with_replyseq}" name="seq" id=hiddenSeq>
						<input type=hidden value="${dto.with_replypseq}" name="pseq" id=hiddenPseq>
						<span> ${dto.with_replyer } (${dto.with_replydate} )</span>
				...
				<tr>
					<td id="reply">
						<textarea id="replytext" maxlength="600" style="height: 60px; width: 100%; resize: none; border: none;" readonly="readonly">${dto.with_replytext}</textarea>
						<div style="float: right;">
							<c:if test="${dto.with_replyer == loginID }">
		...
		<table class="table table-striped" style="text-align: center;">
			<tr>
				<!-- 로그인해야지 댓글창 열립니다 -->
				<c:choose>
					<c:when test="${dto.with_writer == null   }">
					...
					<c:otherwise>
						<input type=hidden value="${dto.with_seq }" id="seq" name="seq">
	...
	<script>
		...
		$("#del").on("click", function() {
			if (confirm("정말 삭제하시겠습니까?")) {
				location.href = "/delete.with?seq=${dto.with_seq}";
			}
		});

		/* '글쓰기' 버튼 누르면 글쓰기 페이지로 넘어가기*/
		$("#write").on("click", function() {
			location.href = "/write.with";
		});

		/* '목록으로' 버튼 누르면 목록으로 돌아가기*/
		$("#list").on("click", function() {
			location.href = "/list.with?cpage=1";
		})
		...
		function modCancle() {
			location.href = "/detail.with?seq=${dto.with_seq}";
		}
	</script>
</body>
