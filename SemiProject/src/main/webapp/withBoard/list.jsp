	...
	<div class="container" style="padding: 10px;">
		...
		<div class="board" style="margin-bottom: 20px;" >
			...
			<c:forEach var="dto" items="${list}" >
				<div class="row" id="hover" style=" height:50px; line-height:50px; 
				border-bottom: 2px solid rgb(202, 200, 200); text-align: left;">
					<div class="col-1 p-0" style="float: left;">${dto.with_seq }</div>
					<div class="col-5 p-0" style="float: left;">
						<a href="/detail.with?seq=${dto.with_seq }">${dto.with_title }</a>
					</div>
		
					
					<div class="col-2 p-0" style="float: left;">${dto.with_writer }</div>
					<div class="col-3 p-0" style="float: left;">${dto.detailDate }</div>
					<div class="col-1 p-0" style="float: left;">${dto.with_viewcount }</div>
				</div>
		...
		<div class="row" style="text-align: center; margin: 10px;">
			...
			<div class="col-1 p-0">
				<input type="button" value=글쓰기  class="btn btn-outline-secondary" onclick="location.href='/write.with'" >
			</div>
			</c:if>
		</div>
		...
		<!-- 검색 기능 -->
		<form action="/search.with?cpage=1" method="post" onkeypress="return event.keyCode!=13"> <!-- 엔터 치면 페이지 넘어가는 것을 방지 -->
