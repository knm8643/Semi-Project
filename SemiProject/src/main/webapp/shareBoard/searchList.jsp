	...
	<div class="container" style="padding: 10px;">
				...
				<c:otherwise>
					...
					<c:forEach var="dto" items="${list}">
						...
						<div class="row" id="hover" style="height: 50px; line-height: 50px; border-bottom: 2px solid rgb(202, 200, 200); text-align: left;">
							<div class="col-1" style="float: left;">${dto.sh_seq }</div>
							<div class="col-5" style="float: left;">
								<a href="/detail.board?seq=${dto.sh_seq }">${dto.sh_title }</a>
							</div>

							<div class="col-2" style="float: left;">${dto.sh_writer }</div>
							<div class="col-3" style="float: left;">${dto.detailDate }</div>
							<div class="col-1" style="float: left;">${dto.sh_viewcount }</div>
						</div>
		...
		<div class="row" style="text-align: center; margin: 10px;">
			...
			<div class="col-2 p-0">
				...
				<input type="button" class="btn btn-outline-secondary" value=글쓰기 onclick="location.href='/shareWrite.board'">
				</c:if>
				<input type="button" class="btn btn-outline-secondary" value=목록으로 onclick="location.href='/list.board?cpage=1'">
		...
		<!-- 검색 기능 -->
		<form action="/search.board?cpage=1" method="post" onkeypress="return event.keyCode!=13">
