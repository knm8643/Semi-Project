...
<body>
	...
	<div class="container" style="padding: 10px;">
		<form action="/modify.board" method="post">
			...
			<div class="row">
				<div class="col">
					<input type="text" id="title" name="title" placeholder="제목을 입력하세요." value="${dto.sh_title }" style="margin: 10px; width: 100%; border: none; font-size: 15px;">
				</div>
			</div>

			<!-- 게시글 내용 -->
			<div class="row" style="border: 1px solid gray;">
				<div class="col" id="content" name="content" style="overflow: auto; width: 100%; height: 500px;">${dto.sh_contents }</div>
			</div>
			...
			<div class="row" style="margin: 20px; text-align: right;">
				<div class="col" style="float: right;">
					...
					<c:if test="${dto.sh_writer == loginID }">
	...
	<div class="container">
		...
		<c:forEach var="dto" items="${list}">
			<table class="table table-striped">
				...
				<tr>
					<td>
						<input type=hidden value="${dto.sh_replyseq}" name="seq" id=hiddenSeq>
						<input type=hidden value="${dto.sh_replypseq}" name="pseq" id=hiddenPseq>
						<span> ${dto.sh_replyer } (${dto.sh_replydate} )</span>
				...
				<tr>
					<td id="reply">
						<textarea id="replytext" maxlength="600" style="height: 60px; width: 100%; resize: none; border: none;" readonly="readonly">${dto.sh_replytext}</textarea>
						<div style="float: right;">
							<c:if test="${dto.sh_replyer == loginID }">
		...
		<table class="table table-striped" style="text-align: center;">
			<tr>
				<!-- 로그인해야지 댓글창 열립니다 -->
				<c:choose>
					<c:when test="${dto.sh_writer == null  }">
					...
					<c:otherwise>
						<input type=hidden value="${dto.sh_seq }" id="seq" name="seq">
	...
	<script>
		...
		$("#del").on("click", function() {
			if (confirm("정말 삭제하시겠습니까?")) {
				location.href = "/delete.board?seq=${dto.sh_seq}";
			}
		});

		/* '글쓰기' 버튼 누르면 글쓰기 페이지로 넘어가기*/
		$("#write").on("click", function() {
			location.href = "/shareWrite.board";
		});
