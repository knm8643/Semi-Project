...
padding: 0px;}
#hover:hover {
	background-color: rgb(245, 243, 112);
...
<body>
	...
	<div class="container" style="padding: 10px;">
		...
		<div class="board" style="margin-bottom: 20px;" >
			...
			<c:forEach var="dto" items="${list}" >
				<div class="row" id="hover" style=" height:50px; line-height:50px; border-bottom: 2px solid rgb(202, 200, 200); text-align: left;">
					<div class="col-1 p-0" style="float: left;">${dto.sh_seq }</div>
					<div class="col-5 p-0" style="float: left;">
						<a href="/detail.board?seq=${dto.sh_seq }">${dto.sh_title }</a>
					</div>
		
					
					<div class="col-2 p-0" style="float: left;">${dto.sh_writer }</div>
					<div class="col-3 p-0" style="float: left;">${dto.detailDate }</div>
					<div class="col-1 p-0" style="float: left;">${dto.sh_viewcount }</div>
				</div>
		...
		<div class="row" style="text-align: center; margin: 10px;">
			<div class="col-11 p-0">${navi }</div>
			<div class="col-1 p-0">
			<c:if test="${blacklist != 'Y'}">
				<input type="button" value=글쓰기  class="btn btn-outline-secondary" id="writebtn" onclick="location.href='/shareWrite.board'" >
			</c:if>
			</div>
		</div>
		...
		<!-- 검색 기능 -->
		<form action="/search.board?cpage=1" method="post" onkeypress="return event.keyCode!=13"> <!-- 엔터 치면 페이지 넘어가는 것을 방지 -->
			<div class="row" style="text-align: center;">
				<div class="col">
					<select name="keyword">
						<option value="sh_title">제목</option>
						<option value="sh_contents">내용</option>
						<option value="sh_writer">작성자</option>
					</select>
					
					<input id="search-input" name="searchWord" type="text" placeholder="검색어를 입력하세요" style="width: 300px;">
